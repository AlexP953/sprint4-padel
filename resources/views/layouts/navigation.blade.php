<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16">
          <div class="flex">
              ...
              <div class="shrink-0 flex items-center">
                  <a href="{{ route('create-reservation') }}">
                      <img class="w-6" src="padelBall.png"/>
                  </a>
              </div>
              <!-- Navigation Links -->
              <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                  <x-nav-link :href="route('create-reservation')" :active="request()->routeIs('create-reservation')">
                      {{ __('Reservar') }}
                  </x-nav-link>
              </div>
        ...
        <form method="POST" action="{{ route('logout') }}" class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
            @csrf
            <x-nav-link :href="route('logout')" :active="request()->routeIs('logout')" onclick="event.preventDefault();
  ...
  <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
      <div class="pt-2 pb-3 space-y-1">
          ...
          <x-responsive-nav-link :href="route('create-reservation')" :active="request()->routeIs('create-reservation')">
              {{ __('Reservar') }}
          </x-responsive-nav-link>
